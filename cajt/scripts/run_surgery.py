...
from cajt.data import (
    load_beavertails_with_categories,
    load_civilcomments_with_identity,
)
from cajt.evaluation.results import print_bias_results
from cajt.intervention import (
    LEACEWrappedModel,
    SuppressedModel,
...
_DATASET_CONFIG = {
    "civilcomments": (load_civilcomments_with_identity, IDENTITY_TOKEN_CANDIDATES),
    "beavertails": (load_beavertails_with_categories, HARM_TOKEN_CANDIDATES),
}


def _load_data(config: Config, dataset_name: str, split: str):
    """Load dataset for surgery experiment.
    ...
    if split == "test":
        _, _, texts, labels, _, _, meta = loader_fn(
            train_samples=0, test_samples=config.data.test_samples, seed=seed,
        )
        return texts, labels, meta, token_candidates
    elif split == "train":
        texts, labels, _, _, _, _, _ = loader_fn(
...
def run(config: Config) -> dict:
    """Run surgical bias removal experiment."""
    ...
    test_texts, test_labels, test_meta, token_candidates = _load_data(config, dataset_name, "test")
    ...
    print_bias_results(bias_before)
    ...
    print_bias_results(bias_after)

    # LEACE baseline: concept erasure via covariance-based projection
    print("\n--- LEACE concept erasure baseline ---")
    train_texts_leace, train_labels_leace = _load_data(config, dataset_name, "train")
