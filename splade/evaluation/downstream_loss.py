...
def compute_downstream_loss(
    ...
    for start in range(0, len(input_ids_list), batch_size):
        ...
        with torch.inference_mode(), torch.amp.autocast("cuda", dtype=COMPUTE_DTYPE):
            ...
            mlm_logits = _model._backbone_forward(
                batch_mask, input_ids=batch_ids,
            ).logits

            if _model.virtual_expander is not None and _model._captured_hidden is not None:
                mlm_logits = _model.virtual_expander(_model._captured_hidden, mlm_logits)

            dense_seq = torch.relu(mlm_logits)
            dense_seq = dense_seq.masked_fill(~batch_mask.unsqueeze(-1).bool(), 0.0)
            if _model.attention_pool is not None:
                dense_pooled = _model.attention_pool(dense_seq, batch_mask)
            else:
                dense_pooled = dense_seq.max(dim=1).values
            dense_logits = _model.classifier_logits_only(dense_pooled)
