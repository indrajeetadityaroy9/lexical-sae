...
def compute_tpp(
    ...
    # Find most discriminative features (highest absolute weight in probe)
    coef_importance = np.abs(clf_full.coef_).sum(axis=0)
    top_features = np.argsort(coef_importance)[-n_target_features:][::-1]
